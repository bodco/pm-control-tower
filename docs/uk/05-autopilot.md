...
> ⚠️ **Урок про локальні задачі.** Enable-стан локальної scheduled task керується лише застосунком Claude Desktop: Claude не може вимкнути її із сесії. Коли локальна `acme-daily-slack-sync` після перейменування скілів почала кликати неіснуючу назву, її тіло замінили заглушкою, але саму задачу довелось вимикати руками в Claude Desktop → Scheduled Tasks. Заглушка в промпті не дорівнює вимкненню, а перейменування двигуна не завершене, поки не перевірені промпти всіх задач (урок у `04`).
...
## Розклад (інстанція автора, Acme, локальний час Europe/Kyiv)

> Розклад нижче - інстанція автора, наведена як приклад. У пакет передачі scheduled tasks не входять: колега створює свої за шаблоном промпта нижче і мінімальним набором з `08` Phase 6.
...
| робочі дні 10/12/14/16/18 | `acme-branch-review` | Скан dev-каналу з конфігу на повідомлення `REVIEW: <гілка>`; якщо є, code review українською у тред; наступні запуски читають відповіді у треді як уточнення і можуть змінити вердикт; якщо нема - миттєвий вихід |

### Тижневі
| Час | Task ID | Що робить |
|---|---|---|
| Пн 06:32 | `topic-manager` (тижневий режим) | Скан Meetings + Threads (і Gmail, якщо налаштовано) на спільні теми → Topics DB |
...
| 1-ше 06:00 UTC | `Automation Health Check (monthly)` | хмара | Аудит усіх власних скілів проти конфігів → Reports DB |
...
Для дорогих задач - явна економія токенів на початку промпта: "Спершу легкий скан; репо, KB і генерацію чіпай ТІЛЬКИ якщо є необроблений тригер; якщо нема - швидко вийди" (`acme-branch-review`). Для інкрементальних - "тільки останні 2 дні, не пересканувати історію" (`daily-current-state-distillation`).
...
| `deploy-analysis-daily`, `acme-branch-review`, `acme-env-audit` (локальні git refs) | `client-satisfaction-bimonthly-1st` / `-15th` |
| `stability-scan` (CloudWatch-експорти на диску) | `monthly-velocity` |
| `daily-work-report` (пише у `~/work/Daily Reports/`) | вже там: Automation Health Check, Monthly Memory Digest |
| prep-задачі: можуть у хмару, якщо Jira доступна хмарним конектором | `daily-current-state-distillation` (тільки Notion); **`Acme daily Slack sync` (перенесено в хмару)**; **`Acme threads -> Jira tickets` (нова хмарна задача, не міграція: Jira-запис через локальний MCP `jira` (`jira.mcp_write` у конфігу))** |

**Крон у хмарі рахується в UTC.** "07:05 за Києвом" це `5 4 * * *` улітку і `5 5 * * *` узимку; без ручної правки двічі на рік усі хмарні задачі зсуваються на годину. Локальний планувальник десктопа працює у локальному часі і цієї проблеми не має. Рішення не прийняте (відкрите питання #42 у `13`).
...
3. **Побічні ефекти у зовнішні системи** - обмежені і явні: Jira-тікети від stability-scan, сторінки у Threads/Topics/Risks/Decisions, коментарі у Slack-треді від acme-branch-review (тільки як відповідь на явний `REVIEW:`), CSV для клієнтського Notion. Нічого не надсилається клієнту автоматично.
...
- Скіл застарів відносно конфігу - Automation Health Check покаже 1-го числа; між аудитами сигнал - дивні імена людей або компонентів у звітах.
...
Cowork (Mac): відкрити Scheduled Tasks у застосунку: список, увімкнути/вимкнути, змінити cron або промпт, Run now. Створення нової задачі: через застосунок.
