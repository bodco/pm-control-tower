...
- Особисті або іншодоменні скіли (`t2-*`, `marken-order`, `ukr-dissertation-format`, `signal-desktop`) живуть поза системою і реєстр не читають.
...
Актуальні URL корпоративних систем. Потрібно, щоб скіли читали старі посилання у тікетах як історичні і не переписували те, що не змінилось (наприклад локальний шлях `/Users/our-company/` - це username, а не домен).

### Task Tracker (режим доступу; у `_template.md` ще немає)
...
Дата і що змінилось. Це "пам'ять про зміни", яку Skill Health Check порівнює зі скілами.
...
- Раз на місяць хмарний Skill Health Check порівнює кожен скіл із конфігами і повідомляє дрейф (`05-autopilot.md`).

## Що дублюється поза конфігом (свідомо, і це треба тримати в синхроні)

1. **Реєстр емейлів у колекторах**: ~~`gmail-collector` і `mac-mail-collector` мають вбудовані таблиці~~.: `gmail-collector` видалено, а `mac-mail-collector` читає секцію `email_routing` з конфігів проєктів. Адреси, спільні між проєктами, більше не ведуться руками: спільною вважається адреса, яка є у `client_emails` двох і більше активних конфігів. Джерело правди одне - конфіг.
2. **Глобальні інструкції Cowork** ("PM Workspace"): таблиця активних проєктів і коротка шпаргалка по дефолтному. Зараз вони ще посилаються на `_projects/`, старий URL Jira і стару команду. Рекомендація рецензії (прийнято як план v0.2): скоротити до рольової моделі і мета-правил (Rule Zero, Default Project Rule, заборона вигадувати факти, мова/стиль) плюс таблиця "проєкт → slug"; усі факти читаються з `projects/`.
...
Ці секції з'являться у `_template.md` після рішення ПМ; поки що це план, скіли їх не читають.

| Секція | Що містить | Що вирішує |
|---|---|---|
| `task_tracker` | тип трекера, `api_access`, `fallback_source`, `export_path`, `browser_access` (описано вище) | Zero-API Access у клієнтів; graceful degradation двигунів. Пріоритет 1 серед розширень |
...
| `email_routing` | усі клієнтські адреси, унікальні, спільні, адреси команди, унікальні домени | Єдиний реєстр замість трьох (конфіг + два колектори); AppleScript експортує все, роутить скіл за конфігом |
| `secrets` | **Зроблено.** У конфігу лише `token_env` і `secrets_file`; значення у `/Users/our-company/work/Secrets/secrets.env` (видима папка, chmod 600, поза синхронізацією). Той самий файл читає локальний Slack MCP-сервер через `sh -c` у `claude_desktop_config.json`. Git-креденшели (GitHub PAT для `pm-control-tower`) живуть поруч, у `Secrets/.git-credentials` - окремий файл, бо git не розуміє формат `KEY=value` (`13` #47). `jira-cosmix`/`jira-rixbeck`/`confluence-our-company` ще не переїхали на цей патерн - `13` #43 | Токени не потрапляють ні у синхронізовані скіли, ні у JSON конфігу десктопа |
...
У `acme.md` записано: Sentry `projects_in_scope: authorizer-api`, решта `out_of_scope` з причиною. Наслідок без жодної правки скілів: stability-scan сканує один проєкт замість семи, client-report не згадує MO/CP помилки, daily-team-prep робить quick check тільки по authorizer-api, risk-register не створює ризики на клієнтські компоненти, velocity-report ставить анотацію про непорівнянність періодів. Саме заради цього ефекту існує реєстр.
