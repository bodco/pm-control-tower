...
4. **Keep the context alive.** Project state changes every week; a config that was not updated the same day makes every report untrue a month later. Hence the Changelog, "as of" dates, the Decisions DB and the monthly Skill Health Check.
...
9. **The Notion API has traps.** 2000 characters per block, exact property names with emoji, no SQL without Enterprise, data source ID instead of database ID. Each one cost hours.
...
12. **Client culture is data.** A Colombian client says "yes" and feels something else. Without cultural calibration, sentiment analysis is meaningless. The Culture Map in the config turned guesses into rules.
...
| A date-stamped fact not checked for 90+ days | silent staleness | Skill Health Check, re-verification |
| A chart across the team-change date with no annotation | reads like a collapse | Metric Continuity warning in the config |

## Security and privacy

**What never goes into skills, `.ai/`, repositories:** credentials, tokens, `.env`, dumps with personal data. The violation that the review marked P0 (a Sentry token in the `acme.md` config, which is synced to the cloud) has been **fixed**: the token moved to `/Users/our-company/work/Secrets/secrets.env` (chmod 600, outside the skill sync), the config keeps only `token_env: PROJECT_SENTRY_TOKEN` and the path to the file, and five skills (`sentry-assistant`, `stability-scan`, `daily-team-prep`, `client-report`, `weekly-overview`) read the value at runtime via `grep`+`cut` and never print it. The token was deliberately not reissued: a PM decision, it is not considered compromised. When the framework is handed over to a colleague, the `Secrets/` folder is not handed over: the colleague creates their own.
...
**Agent permissions:** writing to external systems is limited and explicit: Jira tickets from stability-scan, Notion pages, a comment in a Slack thread only as a reply to an explicit `REVIEW:`. No messages to the client, no changes in production systems without a human.
...
**Corporate changes:** the domain migration (our-company.com → your-company.com, mail → our-company.com) showed that URLs and addresses must live in one place (the config, the Internal Infrastructure section) with the rule "read old links as historical, do not rewrite local paths".
