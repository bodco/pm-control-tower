...
| 5 | Communication with the team | Prep for the internal sync with blockers and action items, automatic code review in a Slack thread on request | `daily-team-prep`, `acme-branch-review`, `notion-meeting-topics` | S | A deliberate ethical boundary (from the review): the agent does not analyze 1-1s or the psychological state of the team. The framework boundary here: an engineer onboarding/offboarding checklist, the date of the last 1-1, technical feedback on tickets. Right now there are only config lines (Former Members), no checklist |
...
| 7 | Quality and stability | A weekly Sentry + CloudWatch scan, tickets for defects, RCA on request, KB-driven code review, a branch audit before regression | `stability-scan` (Thu), `sentry-assistant`, `acme-debug`, `acme-core-code-review`, `acme-env-audit` | S | There has been no QA on the team since 2026-08, so there is no regression checklist skill; test plans are kept in xlsx manually |
...
| 14 | Resources, time, finances | Tempo exports into the client report, estimates from logged time, worklog reconciliation on the second project; the PM Profile has `contract.hours_cap_month`, which enables the hours_burn metric in `client-report` | `client-report`, `acme-jira-estimate-setter`, `beta-worklog-collect/check` | M | Margin, budget burn in money, forecast to complete: 0 (there are only one-off pptx decks about team scaling/budget); capacity planning is manual |
| 15 | Operational support and data | Beneficiary reconciliation across three systems, balance analysis, SQL help, card provider audits, lab and courier orders (other domains) | `acme-beneficiary-audit`, `acme-beneficiary-transactions`, `acme-db-assistant`, `<lab>-order` (two adapters) | S | The client's ops requests (changing a document, blocking) are handled through threads and tickets without a dedicated runbook skill |
...
| 19 | Vendors and external integrations | None. The card provider and the payment network on Acme live as context in the KB and in threads | `acme-db-assistant`, KB | 0 | Provider SLAs, third-party incidents, contractual limits: daily routine for fintech |
| 20 | Compliance, security, audits | Diffused across the Security epic (pentests, payment network compliance) and the `security` label | Jira epic Security | M | GDPR/PCI-DSS/ISO checklists, annual audits, evidence collection: a separate stream for enterprise clients |
...
Questions for the reviewer (an AI reviewer or a fellow PM): are there domains missing from the list entirely; are the levels assessed correctly; which of the gaps are critical for a PM with a different stack (Scrum, a product team, several clients). The answers are collected in `13-open-questions.md`.
