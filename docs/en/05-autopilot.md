...
| Local MCP (jira-cosmix, jira-rixbeck, notion REST, confluence) | yes | through the bridge |
...
| What is there right now | 17 local tasks on Acme (`proj-weekly-report` deleted) | 6: Skill Health Check (monthly), Monthly Memory Digest (monthly), Client Satisfaction (the 1st), **Project Health Check** (the 2nd), **Acme daily Slack sync** (daily at 07:05 Kyiv), **Acme threads -> Jira tickets** (daily at ~09:15 Kyiv) |

> ⚠️ **`acme-daily-slack-sync` (the local one) is still active.** It was asked to be switched off twice (a question about moving it to the cloud, then a self-check), but the enabled state of a task is controlled only by the Claude Desktop app, and Claude Cowork cannot toggle it from here. A check confirmed it: the file has not been edited for a long time and still calls `acme-slack-collector` (renamed) and `thread-ticket-sync` (also an outdated name), which means that every day it either fails silently or duplicates the work of the two cloud Routines above. Claude replaced the body of the prompt with a stub text that does nothing and explains why, to stop the damage until it is switched off, but **the task itself still has to be disabled or deleted by hand in Claude Desktop → Scheduled Tasks**: a stub is not the same thing as being switched off.
...
| working days 10/12/14/16/18 | `authorizer-branch-review` | A scan of `#autorizer-development` for messages of the form `REVIEW: <branch>`; if there is one, a code review in Ukrainian in the thread; later runs read the replies in the thread as clarifications and can change the verdict; if there is none, an immediate exit |

### Weekly
| Time | Task ID | What it does |
|---|---|---|
| Mon 06:32 | `weekly-topics-db-update` | A scan of the Control Tower DBs + Gmail for shared themes → Topics DB |
...
| (disabled) | `proj-weekly-report` | the old React report, replaced by `weekly-overview`, remove from the schedule, keep the prompt as an example in `examples/` |
...
| the 1st, 06:00 UTC | `Skill Health Check (monthly)` | cloud | An audit of all of our own skills against the configs → Reports DB |
...
| `deploy-analysis-daily`, `authorizer-branch-review`, `acme-env-audit` (local git refs) | `client-satisfaction-bimonthly-1st` / `-15th` |
| `stability-scan` (CloudWatch exports on disk) | `monthly-velocity` |
| `daily-work-report` (writes into `~/work/Daily Reports/`) | already there: Skill Health Check, Monthly Memory Digest |
| prep tasks: they could go to the cloud if Jira is reachable through a cloud connector | `daily-current-state-distillation` (Notion only); **`Acme daily Slack sync` (moved to the cloud)**; **`Acme threads -> Jira tickets` (a new cloud task, not a migration: the Jira write goes through `jira-cosmix`, a local MCP)** |

**Cron in the cloud is counted in UTC.** "07:05 Kyiv time" is `5 4 * * *` in summer and `5 5 * * *` in winter; without a manual fix twice a year every cloud task shifts by an hour. The local desktop scheduler works in local time and does not have this problem. The decision has not been made (question 42 in `13`).
...
3. **Side effects in external systems** - limited and explicit: Jira tickets from stability-scan, pages in Threads/Topics/Risks/Decisions, comments in the Slack thread from authorizer-branch-review (only as a reply to an explicit `REVIEW:`), a CSV for the client's Notion. Nothing is sent to the client automatically.
...
- A skill has fallen behind its config - Skill Health Check will show it on the 1st; between audits the signal is odd names of people or components in the reports.
...
Cowork (Mac): open Scheduled Tasks in the app: the list, enable/disable, change the cron or the prompt, Run now. Creating a new task: through the app or the `schedule` skill.
